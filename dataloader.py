...
def load_data(path, split=0.1):
    ...
    train_x, valid_x = train_test_split(image_files, test_size=split_size, random_state=42)
    train_y, valid_y = train_test_split(mask_files, test_size=split_size, random_state=42)
...
def getData(X_shape, flag="train"):
    ...
    if flag == "test":
        for x, y in tqdm(testing_files):
            im = cv2.resize(cv2.imread(os.path.join(image_path,x+".png")), (X_shape, X_shape))[:, :, 0]
            mask = cv2.resize(cv2.imread(os.path.join(mask_path,y+".png")), (X_shape, X_shape))[:, :, 0]
